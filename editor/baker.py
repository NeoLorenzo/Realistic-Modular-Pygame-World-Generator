...
def init_worker(config, luts, chunk_dirs, view_modes, chunk_res, chunk_size_cm):
    ...
    # The generator is now stateless and doesn't need pre-computed maps.
    worker_generator = WorldGenerator(config=config, logger=worker_logger)
    worker_luts = luts
...
def bake_world(config: dict, progress_queue: queue.Queue):
    """
    ...
    try:
        # 2. --- Use Provided Configuration (Rule 1) ---
        ...
        # This generator is only used to get world dimensions and settings.
        main_generator = WorldGenerator(config=world_params, logger=logger)

        # 4. --- Prepare Output Directories & Save Config ---
        base_output_dir = f"baked_worlds/seed_{seed}"
        os.makedirs(base_output_dir, exist_ok=True)
        
        logger.info("Saving world configuration to output directory.")
        config_save_path = os.path.join(base_output_dir, "world_config.json")
        with open(config_save_path, 'w') as f:
            # Save the full config dict that was passed into this function.
            # This includes all UI settings and world generation parameters.
        ...
        init_args = (world_params, luts, chunk_dirs, view_modes, DEFAULTS.CHUNK_RESOLUTION, main_generator.settings['chunk_size_cm'])
        
        progress_queue.put({"status": "running", "progress": 0.0})
        processed_chunks = 0
        ...
        with multiprocessing.Pool(processes=num_workers, initializer=init_worker, initargs=init_args) as pool:
            ...
            for result in tqdm(results_iterator, total=total_chunks, desc="Baking Chunks"):
                ...
                if int(progress * 100) > int(last_reported_progress * 100):
                    progress_queue.put({"status": "running", "progress": progress})
        ...
        os.makedirs(base_output_dir, exist_ok=True) # Ensure dir exists before writing
        ...
        progress_queue.put({"status": "complete", "message": "Bake Complete!"})

    except Exception as e:
        logger.critical("Bake process failed with an exception!", exc_info=True)
        progress_queue.put({"status": "error", "message": f"Bake Failed: {e}"})